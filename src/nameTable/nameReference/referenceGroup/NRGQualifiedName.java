...
public class NRGQualifiedName extends NameReferenceGroup {
	...
	public NRGQualifiedName(String name, SourceCodeLocation location) {
		super(name, location);
		...
		if (firstRef.isResolved()) typeDef = getResultTypeDefinition(firstRef);
			...
			if (firstRef.getReferenceKind() == NameReferenceKind.NRK_VARIABLE) {
				...
				if (firstRef.isResolved()) {
					typeDef = getResultTypeDefinition(firstRef);
//					if (typeDef == null) firstRef.setReferenceKind(NameReferenceKind.NRK_VARIABLE);
		...
		if (fieldRef.isResolved()) {
			...
			} else throw new AssertionError("Unexpected name definition kind[" + nameDefKind + "] of [" + nameDef.toString() + 
