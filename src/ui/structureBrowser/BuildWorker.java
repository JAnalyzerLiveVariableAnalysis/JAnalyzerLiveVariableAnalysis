...
import ui.structureBrowser.tree.ClassNode;
import ui.structureBrowser.tree.FieldNode;
import ui.structureBrowser.tree.MethodNode;
import ui.structureBrowser.tree.NodeKind;
import ui.structureBrowser.tree.PackageNode;
import ui.structureBrowser.tree.ProjectNode;
import ui.structureBrowser.tree.ProjectTree;
import ui.structureBrowser.tree.ProjectTreeNode;
import ui.structureBrowser.tree.VersionNode;
...
//				infoLabel.setVisible(false);
	...
	public void loadProject(ProjectInformation proInfo, List<File> versions) {
		...
		if( n != null) {
			...
			loadVersions(versions, n);
			if(!cancel) {
				manager.projectIds.add(proInfo.getProjectName());
			} else {// 取消的时候，将节点移除
				n.removeFromParent();
				System.gc();
			}
	...
	private ProjectTreeNode loadVersion(File versionDir, ProjectTreeNode parent) {
		...
		SourceCodeParser parser = new SourceCodeParser(versionDir.getAbsolutePath());
		...
		// 获取类文件的数量以供进度条使用
		int progress = rootScope.getTotalNumberOfDefinitions(NameDefinitionKind.NDK_TYPE);
	...
	//				infoLabel.setText("  载入包"+pack.getSimpleName());
					...
					parent.add(packNode);
					List<DetailedTypeDefinition> types = pack.getAllDetailedTypeDefinition();
	...
	//				progressBar.setValue(progressBar.getValue()+1);
					...
					classFilePath = type.getCorrespondingFilePath();
		...
		if (fields != null) {
			...
			for (FieldDefinition field : fields) {
				...
				fieldNode = new FieldNode(NodeKind.FIELD_NODE,
						field.getSimpleName() + "@"
								+ field.getLocation().toFullString(), type, 
								getMetricsString(getAvailableSoftwareSizeMeasureList()));
	...
	private void loadMethods(List<MethodDefinition> methods,
		...
		if (methods != null) {
			...
			for (MethodDefinition method : methods) {
				List<VariableDefinition> vars = method.getParameters();
				// 计算方法的规模度量值
				...
				if (vars != null && vars.size() > 0) {
					...
					methodNode = new MethodNode(NodeKind.METHOD_NODE,
							method.getSimpleName() + "@"
									+ method.getLocation().toFullString(),
							paramsType, metrics);
				} else {
					methodNode = new MethodNode(NodeKind.METHOD_NODE,
							method.getSimpleName() + "@"
									+ method.getLocation().toFullString(), null, metrics);
				}
